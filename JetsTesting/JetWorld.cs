using JetsTesting.Items;
using JetsTesting.Tiles;
...
{
	public class JetWorld : ModWorld
	{
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight) {
			...
			if (ShiniesIndex != -1) {
				...
				tasks.Insert(ShiniesIndex + 1, new PassLegacy("Jet Mod Ores", JetModOres));
			}
        }
        private void JetModOres(GenerationProgress progress) {
			...
			progress.Message = "Jet's Mod Ores";
			...
			for (int k = 0; k < (int)((Main.maxTilesX * Main.maxTilesY) * 6E-05); k++) {
				...
				WorldGen.TileRunner(x, y, WorldGen.genRand.Next(3, 35), WorldGen.genRand.Next(2, 20), ModContent.TileType<ElectriteOre>());
